...
export default function Topbar() {
  const { setSidebarOpen, dark, setDark } = useApp();
  const { user } = useAuth();
  const now = new Date();
  return (
    <header className="sticky top-0 z-30 flex h-[73px] items-center justify-between border-b border-slate-200 bg-white/85 px-4 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/85 md:px-7">
      <div className="flex items-center gap-3">
        <button className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden" onClick={() => setSidebarOpen(true)}>
          <FiMenu className="text-xl" />
        </button>
        <div className="relative hidden md:block">
          <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input className="w-64 rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-4 text-sm outline-none focus:border-blue-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:placeholder:text-slate-500" placeholder="Search patients, reports..." />
        </div>
      </div>
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <button onClick={() => setDark(!dark)} className="rounded-xl p-2.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
          {dark ? <FiSun /> : <FiMoon />}
        </button>
        ...
        <button className="flex items-center gap-2 rounded-xl border border-slate-200 py-1.5 pl-1.5 pr-2.5 dark:border-slate-700">
